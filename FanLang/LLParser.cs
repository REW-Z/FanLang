...
namespace FanLang
{
    ...
    public class LLParser
    {
        ...
        //构造函数  
        public LLParser(List<Token> input)
        {
            this.input = input;
            this.lookahead = 0;
        ...
        private void InitGrammar()
        {
            ...
            foreach (var nont in nonterminals)
            {
                ...
                {
                    Console.WriteLine("     产生式 " + production.ToString() + " 的FIRST集：" + string.Concat(FIRST(production.body).ToArray().Select(symb => symb.name + ",")));
                }
            }
            ...
            foreach (var production in productions)
            {
                ...
                for (int i = 0; i < production.body.Length; ++i)
                {
                    //如果【存在A->αBβ】，那么FIRST(β)中除了ε外所有符号都在FOLLOW(B)中  
                    {
                        ...
                        if (β.Count() > 0)
                        {
                            FOLLOW(B).AddCollection(FIRST(β), exceptedTerminals: new List<Terminal> { null });//排除ε  
                        }
                    }
                    ...
                    //如果【存在A->αB】 或者 【A->αBβ(FIRST(B)包含ε)】，那么FOLLOW(A)中所有的符号都在FOLLOW(B)中    
                    {
                        ...
                        {
                            FOLLOW(B).AddCollection(FOLLOW(A));
                        }
                    }
                }
            }
        ...
        private TerminalCollection FOLLOW(Symbol symbol)
        {
            //无缓存 -> 初始化缓存  （CRE:FOLLOW集无法一次计算完毕）    
            if (symbol.cachedFOLLOW == null)
            {
                symbol.cachedFOLLOW = new TerminalCollection();
        ...
        private void ExecuteNonterminal(Nonterminal nonterminal)
        {
            ...
            var stmtNode = new ParseTree.Node() { isLeaf = false, name = nonterminal.name };
            ...
            foreach (var production in nonterminal.productions)
            {
                if (FIRST(production.body).ContainsTerminal(lookaheadToken.name))
                {
                    ...
                    Console.WriteLine("选择产生式：" + targetProduction.ToString());
                    break;
                }
            }
            //未找到合适产生式 - 最后考虑ε产生式  
                ...
                if (FOLLOW(nonterminal).ContainsTerminal(lookaheadToken.name))
                {
                    ...
                    if (targetProduction == null)
                    {
                        foreach (var p in nonterminal.productions)
                        {
                            ...
                            {
                                targetProduction = p;
                                Console.WriteLine("产生式" + p.ToString() + "可以推导出ε，已选择该产生式");
                                break;
                            }
                        }
                    }
            ...
            for (int i = 0; i < targetProduction.body.Length; ++i)
            {
                ...
                if (symbol == null)//ε  
                {
                    //把ε也加到分析树里?  
                    {
                        var terminalNode = new ParseTree.Node() { isLeaf = true, name = "ε" };
